...
public class EquipmentDAOImpl implements EquipmentDAO {
	...
	public Equipment getObject(Integer id) throws DaoException {
		try {
			logger.info("EquipmentId to be deleted="+id);
	...
	@Override
	public void deleteEquipment(Integer id) throws DaoException {
		try{
			logger.info("EquipmentId to be deleted="+id);
		Equipment equipment = getObject(id);
	...
	public List<EquipmentFilter> getEquipmentFilterDetails() throws DaoException {
		try {
			...
			logger.debug(" Failed SQL!EquipmentDAOImpl in  getEquipmentFilterDetails() " + ex);
			throw new DaoException("Exception in EquipmentDAOImpl in getEquipmentFilterDetails() " + ex);
		}	
	}	
	...
	public void deleteEquipmentFilterDetails(Integer id) throws DaoException {
		try{
			EquipmentFilter equipmentFilter = getObjectEquipmentFilter(id);
			manager.remove(equipmentFilter);
			}catch(Exception ex)
			{
				logger.debug(" Failed To Delete Equipment. " + ex);
				throw new DaoException("Exception in EquipmentImpl in deleteEquipment" + ex);
	...
	public List<ApplicableTestEquipment> getApplicableTestEquipmentDetails() throws DaoException {
		try {
			List<ApplicableTestEquipment> equipmentFilterList = manager.createQuery("Select u From Equipment u", ApplicableTestEquipment.class).getResultList();
			return equipmentFilterList;
		} catch (Exception ex) {
	...
	@Override
	public void deleteApplicableTestEquipmentDetails(Integer id)
			throws DaoException {
		try{
			ApplicableTestEquipment applicableTestEquipment = getObjectApplicableTestEquipment(id);
			manager.remove(applicableTestEquipment);
			}catch(Exception ex)
			{
				logger.debug(" Failed To Delete Equipment. " + ex);
				throw new DaoException("Exception in EquipmentImpl in deleteEquipment" + ex);
